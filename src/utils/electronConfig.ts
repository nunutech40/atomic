export function getElectronShells(atomicNumber: number): number[] {
    ...
    return config[atomicNumber] || [];
}

export function getShellLabel(index: number): string {
    return ['K', 'L', 'M', 'N', 'O', 'P', 'Q'][index] || `S${index + 1}`;
}

export function getMaxElectronsInShell(n: number): number {
    return 2 * (n + 1) * (n + 1);
}
